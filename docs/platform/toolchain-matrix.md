...
| Linux (x86, glibc) | GCC toolchain producing 32-bit `qagamei386.so` | Reference `.so` files exist; reproducible build configuration still pending | glibc-based userland (`libc6`)<br>X11 with DGA & VidMode extensions<br>Vendor OpenGL drivers (Mesa + Glide for 3dfx era) | Historical README documents the 32-bit runtime expectations; align the build with those requirements and confirm against the archived shared objects.【F:docs/reference-mapping.md†L18-L24】【F:src/code/unix/README.Linux†L68-L155】【F:src/README.txt†L180-L188】 |
...
- Define and document a reproducible Linux 32-bit build configuration that emits `qagamei386.so` against a known glibc baseline, mirroring the archived binaries in the repository.【F:docs/reference-mapping.md†L18-L24】【F:src/code/unix/README.Linux†L68-L155】

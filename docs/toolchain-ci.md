...
- `.github/workflows/nightly-build.yml` runs scheduled Linux/macOS native POSIX
  ...
  checksums, and manifest metadata to a prerelease GitHub Release as well as
  workflow artifacts.
...
nightly prerelease in GitHub Releases. Linux jobs build a native POSIX package
...
publishes those files plus `version.json` as release assets. The release is a
prerelease tagged with the nightly artifact version and is updated in place when
the same workflow run is retried. The packages intentionally do not include
...
audits. The hosted Windows modern lane disables optional OGG, PNG, and FreeType
bootstraps so nightlies do not depend on ignored local codec/font import
libraries; full codec-enabled validation remains a local/dependency-staged
toolchain exercise.

...
indexes the uploaded archives with SHA256 sums. Linux jobs build a native POSIX
package with `.so` module outputs, macOS jobs build a native POSIX package with
`.dylib` module outputs, and the Windows package job uses the
...
for all uploaded nightly archives. The package intentionally does not include

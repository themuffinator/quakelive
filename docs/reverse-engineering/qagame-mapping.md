...
- `G_RRApplySurvivalBonus` at `0x100652B0` and `G_RRCheckInfectionSpread` at `0x10065410` are descriptive retail-only Red Rover infection helper names. The current source now mirrors the survival-bonus timer/forced-award split, the survivor-only bonus print, and the global infection-spread countdown/forced conversion order; the broadcast temp-entity half of the survival bonus is still deferred until the matching cgame consumer is recovered.

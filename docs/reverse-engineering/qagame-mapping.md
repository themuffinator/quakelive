...
| `0x100549E0` | `G_InitPublishedCvarState` | Retail-only `G_InitGame` sidecar | Called immediately after `G_RegisterCvars`; it seeds the published player-cylinder, factory-flags, custom-settings, and loadout configstring slab plus the retail Domination/Red Rover numeric configstrings and snapshots the related cvar mirrors used later in gameplay. | High |
...
| `0x1006B210` | `G_NotifyLastAlivePlayer` | Retail-only shared round alert helper | Locates the sole surviving `PM_NORMAL` client on the selected team, emits the paired temp-entity/event, and sends that client the preserved centerprint notification reused by A/D, Clan Arena, Freeze, and Red Rover. The current source now stages the private centerprint half for A/D, Clan Arena, and Freeze while leaving the public temp-entity/POI bridge open. | High |
...
- `G_RunFactoryHealthRegen` at `0x10034260` and `G_RunFactoryArmorRegen` at `0x100342F0` are descriptive retail-only helpers keyed off a shared last-damage timestamp plus separate client-side accumulators/latches. The reconstructed tree now preserves the split helper flow and the retail spawn/damage latch-arming behavior, but some adjacent `ClientTimerActions` client-state fields are still only partially named.
...
- `G_ADShouldTimeoutActiveRound`, `G_ADNotifyLastAlivePlayer`, `G_CAResolveRoundState`, `G_CAHandleDamageScore`, `G_CANotifyLastAlivePlayer`, `G_FreezeNotifyLastAlivePlayer`, and `G_NotifyLastAlivePlayer` are likewise descriptive retail-only helper names. The current tree now preserves the private last-man-standing centerprint helper boundaries for A/D, Clan Arena, Freeze, and Red Rover, but the paired public temp-entity/POI alert path remains open.
...
- `G_RRCheckRoundCompletion` at `0x10064670` is a descriptive retail-only helper name. The current source now preserves the carryover infected-slot latch, the retail round-score increment, and the delayed post-round exit-vs-restart split, but the draw-delay gate and the full connected-count interface are still not preserved exactly.
- `G_RRCheckExitRules` at `0x10064710` is a descriptive retail-only helper name. The current source now preserves the standalone tie-aware RR timelimit/roundlimit gate and the retail `Timelimit hit.` / `Roundlimit hit.` logging path, but it still stages that check through the simplified GPL-derived round controller instead of the exact six-state retail machine.
...
- `G_RRResolveAutoJoinTeam` at `0x100647C0` and `G_RRSeedInfectionTeams` at `0x10064820` are descriptive retail-only infection helpers. The current source now mirrors the preserved infected-slot handoff through `SetTeam` and the round-start reseed back to `TEAM_BLUE` plus one promoted `TEAM_RED` infected client, but the retail pending-transition resolver is still folded into broader source control flow.
...
- `G_RRTrackRoundActivity` at `0x100655A0` is source-faithful on its outer boundary, and the current source now follows the retail survival-bonus then infection-spread then completion ordering, but retail still folds more controller-transition work into the deeper helper chain than the GPL-derived implementation preserves.
...
- `G_RRHandlePlayerDeath` at `0x10065700` is source-faithful on role but not on exact interface. The current source now flips the victim's team in infection mode, refreshes userinfo, emits the retail `EV_INFECTED` single-recipient temp entity, and reuses the shared Red Rover last-survivor warning, but retail still receives the victim's pre-mutation team and extra bookkeeping args directly from the death path.
...
- No high-confidence unresolved function seams remain in the current curated qagame control surface. The remaining reverse-engineering work is now mostly adjacent client-state recovery around `ClientTimerActions`, especially the still-unidentified timer accumulators neighboring the confirmed factory regen state (for example the `gclient + 0x500` / `+0x50C` fields visible in HLIL).

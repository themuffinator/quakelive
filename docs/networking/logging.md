...
* When the backend denies the ticket, the helper now writes a `SteamAuthRejected` log before returning the denial string so the server trace mirrors Quake Live's failure hooks.【F:src/code/game/g_client.c†L123-L139】
